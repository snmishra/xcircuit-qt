...
static QAction *param_buttons[15];
...
void param_init()
{
    param_buttons[0] = menuAction("Parameters_Numeric");   /* P_NUMERIC */
    param_buttons[1] = menuAction("Parameters_Substring"); /* P_SUBSTRING */
    param_buttons[2] = menuAction("Parameters_Position");  /* P_POSITION_X */
    param_buttons[3] = menuAction("Parameters_Position");  /* P_POSITION_Y */
    param_buttons[4] = menuAction("Parameters_Style");     /* P_STYLE */
    param_buttons[5] = menuAction("Parameters_Justification"); /* P_JUSTIFY */
    param_buttons[6] = menuAction("Parameters_StartAngle");    /* P_ANGLE1 */
    param_buttons[7] = menuAction("Parameters_EndAngle");  /* P_ANGLE2 */
    param_buttons[8] = menuAction("Parameters_Radius");    /* P_RADIUS */
    param_buttons[9] = menuAction("Parameters_MinorAxis"); /* P_MINOR_AXIS */
    param_buttons[10] = menuAction("Parameters_Rotation");  /* P_ROTATION */
    param_buttons[11] = menuAction("Parameters_Scale");     /* P_SCALE */
    param_buttons[12] = menuAction("Parameters_Linewidth"); /* P_LINEWIDTH */
    param_buttons[13] = menuAction("Parameters_Color");     /* P_COLOR */
    param_buttons[14] = menuAction("Parameters_Position");  /* P_POSITION */
}
...
bool check_param(objectptr thisobj, char *key)
{
   oparamptr tops;
...
oparamptr match_param(const object* thisobj, const char *key)
{
   oparamptr fparam;
...
oparamptr match_instance_param(objinstptr thisinst, const char *key)
{
   oparamptr fparam;
...
oparamptr find_param(objinstptr thisinst, const char *key)
{
...
void setparammarks(genericptr thiselem)
{
   ...
   for (i = 0; i < rlength; i++) {
       param_buttons[i]->setChecked(false);
   }
   ...
   if (thiselem != NULL) {
      for (epp = thiselem->passed; epp != NULL; epp = epp->next) {
         ops = match_param(topobject, epp->key);
         param_buttons[ops->which]->setChecked(true);
      }
   }
   ...
   for (i = 0; i < rlength; i++) {
      if ((thiselem == NULL) || (param_select[i] & thiselem->type))
         param_buttons[i]->setEnabled(true);
      else
         param_buttons[i]->setEnabled(false);
   }
}
...
int opsubstitute(objectptr thisobj, objinstptr pinst)
{
   ...
   stringpart *strptr;
   int retval = -1;
   bool needrecalc;	/* for arcs and splines */
   ...
   for (eptr = thisobj->begin(); eptr != thisobj->end(); eptr++) {
...
bool has_param(genericptr celem)
{
...
void pwriteback(objinstptr thisinst)
{
   ...
   XPoint *setpt;
   bool changed, need_redraw = false;
   union {
      int ival;
      float fval;
   } wtemp;
...
void resolveparams(objinstptr thisinst)
{
   ...
   if ((i = checklibtop()) >= 0) {
      ...
      if ((spec == NULL) || (spec->isvirtual == false)) {
...
void copyalleparams(genericptr destinst, const generic* sourceinst)
{
...
void copyparams(objinstptr destinst, const objinst *sourceinst)
{
...
bool makefloatparam(objectptr refobject, char *key, float value)
{
   oparamptr newops;
   char *newkey;
...
bool makestringparam(objectptr refobject, char *key, stringpart *strptr)
{
   oparamptr newops;
   char *newkey;
...
const char *getnumericalpkey(u_int mode)
{
   ...
   if (newops) {
	 ...
	 newops->parameter.ivalue = (int)((*gelem)->color);
      ...
      switch((*gelem)->type) {
	    ...
	    break;
         case POLYGON:
            if (loccycle == -1) loccycle = (TOPOLY(gelem)->cycle != NULL) ?
   ...
   while (!done) {
	 ...
	 if (ops == NULL) break;	/* Error---no such parameter */
	    ...
	    /* remove the parameter itself from the object.		  */
            ...
            for (genericptr* pgen = topobject->begin(); pgen != topobject->end(); ++pgen) {
...
void searchinst(objectptr topobj, objectptr refobj, char *key)
{
   ...
   for (objinstiter tinst; topobj->values(tinst); ) {
	 ...
	 if (tmpptr->type == PARAM_START) lastptr = tmpptr;
      ...
      for (fselect = areawin->selectlist; fselect < areawin->selectlist +
	    ...
	    if (strptr != NULL) unmakeparam(settext, strptr);
            ...
            removenumericalp(topobject->begin() + (*fselect), P_POSITION_X);
            removenumericalp(topobject->begin() + (*fselect), P_POSITION_Y);
	 }
	 else
            removenumericalp(topobject->begin() + (*fselect), mode);
      }
      setparammarks(NULL);
   }
}
   ...
   for (fselect = areawin->selectlist; fselect < areawin->selectlist +
      ...
      if ((mode == P_SUBSTRING) && (areawin->selects == 1) &&
		(SELECTTYPE(fselect) == LABEL)) {
         ...
         makenumericalp(topobject->begin() + (*fselect), P_POSITION_X, key, cycle);
         makenumericalp(topobject->begin() + (*fselect), P_POSITION_Y, key, cycle);
      }
      else
