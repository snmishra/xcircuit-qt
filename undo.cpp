...
struct editelement {
   ...
   union {
      ...
      pointlist *pathspecs;	/* original parts of a path */
...
editelement::~editelement()
{
    switch (element->type) {
       case LABEL:
          freelabel(save.string);
          break;
       case POLYGON: case SPLINE:
          delete save.points;
          break;
       case ARC:
          delete save.arcspecs;
          break;
       case PATH:
          delete [] save.pathspecs;
          break;
    }
}
...
bool setwindow(XCWindowData *trywindow)
{
   XCWindowData *chkwin;
...
uselection::uselection(objinstptr topinst, short *slist, int n) :
        number(n),
        element(n > 0 ? new genericptr[n] : NULL),
        idx(n > 0 ? new short[n] : NULL)
{
    if (n > 0) {
        for (int i = 0; i < number; ++i) {
            const int j = slist[i];
            element[i] = topinst->thisobject->at(j);
            idx[i] = j;
        }
    }
}
...
/*----------------------------------------------------------------------*/
/* Create a selection list in from the saved uselection		*/
/* record.								*/
/*----------------------------------------------------------------------*/

short * uselection::regen(objinstptr thisinst)
{
   int j, k;
   ...
   if (number > 0)
      slist = (short *)malloc(number * sizeof(short));

   k = 0;
   for (int i = 0; i < number; i++) {
      /* Use the element address, not the selection order. */
      genericptr egen = element[i];
      if (egen == thisobj->at(idx[i])) {
         // this is a short-circuit, we use idx[] as a cache of indices into
         // the object's plist
         j = idx[i];
      }
      else {
         // the short-circuit has failed, we search for the element
         reorder = true;
         for (j = 0; j < thisobj->parts; j++) {
            if (egen == thisobj->at(j))
	       break;
	 }
      }
      if (j < thisobj->parts) {
...
void free_editelement(Undoptr thisrecord)
{
   ...
   switch (erec->element->type) {
      case LABEL:
         delete erec->save.string;
         break;
      case POLYGON: case SPLINE:
         delete erec->save.points;
	 break;
      case ARC:
         delete erec->save.arcspecs;
	 break;
      case PATH:
         delete erec->save.pathspecs;
	 break;
   case OBJINST:
         delete erec->save.instpos;
         break;
   }
   delete erec;
}
   ...
   for (chkrecord = thisrecord->next; chkrecord != NULL; chkrecord = chkrecord->next) {
	 ...
	 case XCF_Select:
	    srec = (uselection *)chkrecord->undodata;
            areawin->selectlist = srec->regen(thisrecord->thisinst);
   ...
   switch(type) {
	 ...
	 *idata = nval;
	 ...
	 snum = va_arg(args, int);
         srec = new uselection(thisinst, slist, snum);
	 ...
	 switch(egen->type) {
	    case LABEL:
	       ...
	       break;
	    case POLYGON:
               newrecord->idata = ((polyptr)egen)->points.count();
               erec->save.points = new pointlist(((polyptr)egen)->points);
	       break;
	    case SPLINE:
               erec->save.points = new pointlist(((splineptr)egen)->ctrl, 4);
	       break;
            case OBJINST:
               erec->save.instpos = new XPoint(((objinstptr)egen)->position);
               break;
	    case ARC:
               erec->save.arcspecs = new arcinfo;
	    ...
	    case PATH:
	       newrecord->idata = ((pathptr)egen)->parts;  /* not needed? */
               erec->save.pathspecs = new pointlist[newrecord->idata];
	       for (i = 0; i < newrecord->idata; i++) {
                  pointlist *points = erec->save.pathspecs + i;
                  genericptr *pgen = ((pathptr)egen)->begin() + i;
		  switch (ELEMENTTYPE(*pgen)) {
		     case POLYGON:
                       *points = (TOPOLY(pgen))->points;
			break;
		     case SPLINE:
                       *points = pointlist((TOSPLINE(pgen))->ctrl, 4);
			break;
		  }
	       }
	       break;
	 ...
	 newrecord->undodata = (char *)malloc(sizeof(XPoint));
         *((XPoint *)newrecord->undodata) = *fpoint;
	 break;
	 ...
	 newrecord->undodata = (char *)malloc(sizeof(XPoint));
         *((XPoint *)newrecord->undodata) = XPoint(deltax, deltay);
  ...
  Undoptr thisrecord;
   ...
   switch(thisrecord->type) {
	 ...
	 areawin->selectlist = xc_undelete(thisrecord->thisinst,
                        thisobj, recover_selectlist(thisrecord));
         srec = new uselection(thisrecord->thisinst, areawin->selectlist,
			areawin->selects);
	 ...
	 i = thisrecord->thisinst->thisobject->parts - 1;
         if (thisinst != *(thisrecord->thisinst->thisobject->begin() + i)) {
	 ...
	 i = thisrecord->thisinst->thisobject->parts - 1;
         if (egen != *(thisrecord->thisinst->thisobject->begin() + i)) {
	    Fprintf(stderr, "Error: No such element!\n");
	    thisrecord->undodata = NULL;
	 }
	 else {
	 ...
	 switch (erec->element->type) {
	    ...
	    case POLYGON: {
	       ...
	       tmppts = epoly->points;
               ...
               delete erec->save.points;
               erec->save.points = new pointlist(tmppts);
               thisrecord->idata = tmppts.count();
	    } break;
            case SPLINE: {
	       splineptr espline = (splineptr)(erec->element);
               pointlist tmppts(espline->ctrl, 4);
	       for (i = 0; i < 4; i++)
	    ...
	    case PATH: {
	       ...
	       for (i = 0; i < epath->parts; i++) {
                  genericptr ggen = *(epath->begin() + i);
		  switch (ELEMENTTYPE(ggen)) {
		     case POLYGON:
			epoly = (polyptr)ggen;
		        tmppts = epoly->points;
                        epoly->points = *(erec->save.pathspecs + i);
                        *(erec->save.pathspecs + i) = tmppts;
			break;
		     case SPLINE:
			espline = (splineptr)ggen;
                        tmppts = pointlist(espline->ctrl, 4);
			for (j = 0; j < 4; j++)
                           espline->ctrl[j] = *(*(erec->save.pathspecs + i) + j);
                        *(erec->save.pathspecs + i) = tmppts;
                        espline->calc();
			break;
		  }
	 ...
	 srec = (uselection *)thisrecord->undodata;
         slist = srec->regen(thisrecord->thisinst);
...
short redo_one_action()
{
   ...
   switch(thisrecord->type) {
	 ...
	 if (thisobj != NULL) {
	    unselect_all();
	    snum = thisobj->parts;
            slist = xc_undelete(thisrecord->thisinst, thisobj, NULL);
            thisrecord->undodata = (char *)new uselection(thisrecord->thisinst,
			slist, snum);
	    free(slist);
	 }
	 break;
	 ...
	 if (thisobj != NULL) {
	    unselect_all();
	    areawin->selects = thisobj->parts;
            areawin->selectlist = xc_undelete(thisrecord->thisinst, thisobj, NULL);
            thisrecord->undodata = (char *)new uselection(thisrecord->thisinst,
	 ...
	 switch (erec->element->type) {
	    ...
	    case POLYGON: {
	       ...
	       tmppts = epoly->points;
            ...
            case SPLINE: {
	       splineptr espline = (splineptr)(erec->element);
               pointlist* tmppts = new pointlist(espline->ctrl, 4);
	       for (i = 0; i < 4; i++)
                   espline->ctrl[i] = (*(pointlist*)erec->save.points)[i];
               delete erec->save.points;
	       erec->save.points = tmppts;
               espline->calc();
	    } break;
   ...
   switch (type) {
	 ...
	 if (mode == MODE_UNDO) {
	    uobj = (objectptr)thisrecord->undodata;
            ...
            delete (uselection *)thisrecord->undodata;
	 }
	 break;
...
/* Calls from the Xt menus (see menus.cpp)				*/
