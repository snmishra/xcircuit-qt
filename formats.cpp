...
void loadlgf(int mode)
{
	 ...
	 case 'l': {	/* labels */
	    ...
	    for (i = 0; i < dval; i++) {
		  ...
		  stringpart *strptr;
                  ...
                  newlabel = topobject->append(new label(false, XPoint(xmat(pvalx), ymat(pvaly))));
	...
	case 'w': {	/* wires, implemented as single-segment polygons */
	    ...
	    for (i = 0; i < dval; i++) {
	       ...
	       /* Allocate wire */
               ...
               newwire = topobject->append(new polygon);
	       ...
	       (*newwire)->style = UNCLOSED;
               ...
               (*newwire)->points[0] = XPoint(xmat(pvalx), ymat(pvaly));
               (*newwire)->points[1] = XPoint(xmat(pvalx2), ymat(pvaly2));
	    }}
	    break;
	    ...
	    for (i = 0; i < dval; i++) {
	       do {
		     ...
		     return;
               ...
               topobject->append(new polygon);

	       (*newpoly)->style = DASHED;
	       (*newpoly)->color = DEFAULTCOLOR;
               (*newpoly)->width = 1.0;
               (*newpoly)->points.resize(4);

               newpoints = (*newpoly)->points.begin();
	       sscanf(lineptr, "%hd %hd %hd %hd", &pvalx, &pvaly, &pvalx2, &pvaly2);
	    ...
	    for (i = 0; i < dval; i++) {
		  ...
		  return;
	       ...
	       if (j < xobjs.numlibs || k < xobjs.userlibs[xobjs.numlibs - 1].number) {
                  ...
                  newinst = topobject->append(new objinst);
		  ...
		  (*newinst)->bbox.height = (*libobj)->bbox.height;
		     ...
		     for (k = 0; k < sigs; k++)
			   ...
			   stringpart *strptr;
                           ...
                           newlabel = topobject->append(new label(false, (*newinst)->position));
			   /* reconnect newinst if displaced by realloc() */
                           /// \todo this may interact with constructor above
   ...
   for (labeliter iolabel; topobject->values(iolabel); ) {
         if (iolabel->rotation == 500) {
	    genericptr *tmplabel;

            delete TOLABEL(iolabel);
