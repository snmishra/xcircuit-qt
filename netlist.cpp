...
static const char *spice_devname = "X";   /* SPICE subcircuit device name */
bool spice_end = true;	    /* whether or not to write a .end statement */
ino_t *included_files = NULL;	/* Files included with "%F" escape	*/

static inline int EndPoint(int n) { return (n == 1) ? 1 : (n - 1); }
static inline int NextPoint(int n) { return (n == 1) ? 0 : 1; }
...
int NameToPinLocation(objinstptr thisinst, char *pinname, int *x_ret, int *y_ret)
{
   ...
   for (labeliter plab; thisobj->values(plab); ) {
...
void ReferencePosition(objinstptr thisinst, XPoint *thispoint, XPoint *refpoint)
{
   ...
   locctm.preMult(thisinst->position, thisinst->scale, thisinst->rotation);
   locctm.transform(thispoint, refpoint, 1);
}
...
bool PortToPosition(objinstptr thisinst, int portno, XPoint *refpoint)
{
...
bool getnexthier(pushlistptr stack, char **hierstr, objinstptr callinst,
	bool canonical)
{
   ...
   if (stack->next != NULL) {
      if (! getnexthier(stack->next, hierstr, stack->thisinst, canonical))
	 return false;
   }
   else {
...
*/
   ...
   for (calls = thisobj->calls; calls != NULL; calls = calls->next) {
	 ...
	 if (*hierstr == NULL) {
            *hierstr = (char*)malloc(devlen);
	    hierlen = 0;
	 }
	 else {
	    hierlen = strlen(*hierstr) + 2;
            *hierstr = (char*)realloc(*hierstr, hierlen + devlen);
	 }
	 if (canonical)
...
char *GetHierarchy(pushlistptr *stackptr, bool canonical)
{
   bool pushed_top = false;
   char *snew = NULL;

   if ((*stackptr) && ((*stackptr)->thisinst != areawin->topinstance)) {
      pushed_top = true;
      push_stack(stackptr, areawin->topinstance);
   }
...
bool proximity(XPoint *point1, XPoint *point2)
{
   int dx, dy;
...
void createnets(objinstptr thisinst, bool quiet)
{
...
void gennetlist(objinstptr thisinst)
{
   ...
   if (!visited && (thisobject->schemtype == SYMBOL)
	 ...
	 if (n == -1) {
      ...
      bool hit = false;
      for (labeliter clab; thisobject->values(clab); ) {
   ...
   for (j = 0; j < xobjs.pages; j++) {
	 ...
	 for (i = 0; i < old_parts; i++) {
		  ...
		  tmplist = (Genericlist *)lseek;
                  for (endpt = cpoly->points.begin(); endpt < cpoly->points.begin()
                                + EndPoint(cpoly->points.count()); endpt++) {
                     endpt2 = endpt + NextPoint(cpoly->points.count());
		  ...
		  if (plist->cschem != cschem) continue;
                  ...
                  tpt = tpoly->points.begin();
                  tpt2 = tpoly->points.end() - 1;
		  tmplist = (Genericlist *)plist;
			...
			if (resolved_net != NULL) {
	          ...
	          /* to each segment of every recorded network polygon.	   */
                  ...
                  endpt = cpoly->points.begin();
                  endpt2 = cpoly->points.end() - 1;
                  for (tpt = tpoly->points.begin(); tpt < tpoly->points.begin()
                           + EndPoint(tpoly->points.count()); tpt++) {
                     tpt2 = tpt + NextPoint(tpoly->points.count());
	...
	short llx, short lly, short urx, short ury)
{
   ...
   if (sibling->symschem != NULL || sibling->schemtype == FUNDAMENTAL
	 ...
	 olabel = lseek->label;
         ...
         tmppts.resize(1);
         UTransformPoints(&(olabel->position), tmppts.begin(), tmppts.count(),
	 ...
	 for (psearch = schemtop; psearch != NULL; psearch = psearch->next) {
	    subsibinst = psearch->thisinst;
            UTransformPoints(tmppts.begin(), tmppts.begin(), tmppts.count(), subsibinst->position,
				subsibinst->scale, subsibinst->rotation);
	 }
         searchconnect(tmppts.begin(), tmppts.count(), cinst, lseek->subnets);
      }
   }
   ...
   /* polygons, and recursively search subschematics of the sibling.	*/
	 ...
	 olabel = lseek->label;
         tmppts.resize(1);
         UTransformPoints(&(olabel->position), tmppts.begin(), tmppts.size(),
			isib->position, isib->scale, isib->rotation);

	 /* Transform all the way up to the level above cinst */
	 for (psearch = schemtop; psearch != NULL; psearch = psearch->next) {
	    subsibinst = psearch->thisinst;
            UTransformPoints(tmppts.begin(), tmppts.begin(), tmppts.size(), subsibinst->position,
			subsibinst->scale, subsibinst->rotation);
	 }
         searchconnect(tmppts.begin(), tmppts.size(), cinst, lseek->subnets);
      }
	 ...
	 tpoly = pseek->poly;
         tmppts.resize(tpoly->points.count());
         UTransformPoints(tpoly->points.begin(), tmppts.begin(), tpoly->points.count(),
				isib->position, isib->scale, isib->rotation);
	 ...
	 for (psearch = schemtop; psearch != NULL; psearch = psearch->next) {
	    subsibinst = psearch->thisinst;
         ...
         searchconnect(tmppts.begin(), tpoly->points.count(), cinst, pseek->subnets);
      }

      /* Recursively search all schematic children of the sibling */

      for (i = 0; i < sibling->parts; i++) {
         iseek = sibling->begin() + i;
         if (IS_OBJINST(*iseek)) {
	 ...
	 cschem = thisobject;
      ...
      for (i = 0; i < cschem->parts; i++) {
	    ...
	    if (callsymb->symschem == NULL
		  ...
		  tpoly = pseek->poly;
                  searchconnect(tpoly->points.begin(), tpoly->points.count(), callinst, pseek->subnets);
	       }
	    ...
	    /*----------------------------------------------------------*/
            ...
            locctm.reset();
            locctm.preMult(callinst->position, callinst->scale,
				callinst->rotation);
		  ...
		  continue;
	       ...
	       /* Translate pin position back to object cschem */
               locctm.transform(&(olabel->position), &xpos, 1);
   ...
   for (calls = pschem->calls; calls != NULL; calls = calls->next) {
      ...
      for (ports = calls->ports; ports != NULL; ports = ports->next) {
	    ...
	    /* objects acting as their own schematics.			 	*/
            ...
            ctx->UPushCTM();
            ctx->CTM().preMult(ccinst->position, ccinst->scale, ccinst->rotation);
	    ...
	    if (ccinst->thisobject->symschem == NULL &&
	       ...
	       /* Fprintf(stdout, " > Net translation from %d to %d (port %d)\n",
		    netid, netto, ports->portid); */
               ...
               if (highlightnet(ctx, calls->callobj, calls->callinst, netto))
		  rval = true;
	    }
	    else {
...
void highlightnetlist(DrawContext* ctx, objectptr nettop, objinstptr cinst)
{
   ...
   for (lbus = 0;; ) {
      ...
      highlightnet(ctx, nettop, nextinst, netid);
      if (++lbus >= netlist->subnets) break;
   }
   areawin->update();
}

/* Remove the netlist entry from the object */
void remove_highlights(objinstptr cinst)
{
    Genericlist *netlist = cinst->thisobject->highlight.netlist;
    freegenlist(netlist);
...
bool match_buses(Genericlist *list1, Genericlist *list2, int mode)
{
   int i;
   buslist *bus1, *bus2;
...
void copy_bus(Genericlist *dest, Genericlist *source)
{
   ...
   if (source->subnets == 0)
      ...
      dest->net.list = new buslist[dest->subnets];
...
Genericlist *new_tmp_pin(objectptr cschem, XPoint *pinpt, char *pinstring,
                const char *prefix, Genericlist *netlist)
{
   labelptr *newlabel;
   stringpart *strptr;
   ...
   newlabel = cschem->append(new label(LOCAL, *pinpt));
   (*newlabel)->justify = 0;
...
labelptr new_global_pin(labelptr clabel, objinstptr cinst)
{
   ...
   newlabel = new label(GLOBAL, XPoint(0, 0));
   newlabel->justify = 0;
   newlabel->color = DEFAULTCOLOR;
...
void promote_net(objectptr cschem, Genericlist *netfrom, int subnets)
{
   ...
   for (plist = cschem->polygons; plist != NULL; plist = plist->next)
	 ...
	 plist->subnets = subnets;
         plist->net.list = new buslist[subnets];
   ...
   for (llist = cschem->labels; llist != NULL; llist = llist->next)
	 ...
	 llist->subnets = subnets;
         llist->net.list = new buslist[subnets];
	 for (lbus = 0; lbus < subnets; lbus++) {
...
bool mergenetlist(objectptr cschem, Genericlist *testlist,
...
bool netmerge(objectptr cschem, Genericlist *orignet, Genericlist *newnet)
{
   PolylistPtr plist;
   LabellistPtr llist;
   CalllistPtr calls;
...
bool mergenets(objectptr cschem, Genericlist *orignet, Genericlist *newnet)
{
   bool merged;
...
bool addportcall(objectptr cschem, Genericlist *netfrom, Genericlist *netto)
{
   CalllistPtr ccall;
...
Genericlist *pointtonet(objectptr cschem, objinstptr cinst, XPoint *testpoint)
{
   ...
   for (ppoly = pschem->polygons; ppoly != NULL; ppoly = ppoly->next) {
      ...
      for (tpt = ppoly->poly->points.begin(); tpt < ppoly->poly->points.begin()
                + EndPoint(ppoly->poly->points.count()); tpt++) {
         tpt2 = tpt + NextPoint(ppoly->poly->points.count());
...
void makelocalpins(objectptr cschem, CalllistPtr clist, const char *prefix)
{
   NetnamePtr netnames;
...
XPoint *NetToPosition(int netid, objectptr cschem)
{
   ...
   for (; plist != NULL; plist = plist->next) {
	 ...
	 if (locnetid == netid) {
            return plist->poly->points.begin();
	 }
	 if (++lbus >= plist->subnets) break;
      }
   }
...
stringpart *nettopin(int netid, objectptr cschem, const char *prefix)
{
   NetnamePtr netname;
   labelptr pinlab;
...
bool samepart(CalllistPtr clist1, CalllistPtr clist2)
{
   PortlistPtr port;
   labelptr plab;
...
LabellistPtr geninfolist(objectptr cschem, objinstptr cinst, const char *mode)
{
   int locpos, j;
   int vmax;
   ...
   for (labeliter plabel; cschem->values(plabel); ) {
...
char *parseinfo(objectptr cfrom, objectptr cthis, CalllistPtr clist,
        const char *prefix, const char *mode, bool autonumber, bool no_output)
{
   ...
   int portid, locpos, i, k, subnet, slen;
   char *key = NULL;
   u_int newindex;
   bool is_flat = false, is_symbol = false, is_iso = false, is_quoted;
   bool do_update = true, include_once = false;
   const char *locmode, *b36str, *sptr;
		  ...
		  case 'f':
			...
			fnsh++; 
		     ...
		     /* Do variable and tilde expansion on the filename */
                     {
                         QString str = QString::fromLocal8Bit(_STR);
                         xc_tilde_expand(str);
                         xc_variable_expand(str);
                         qstrncpy(_STR, str.toLocal8Bit(), sizeof(_STR));
                     }
...
int writedevice(FILE *fp, const char *mode, objectptr cfrom, CalllistPtr clist,
        const char *prefix)
{
   char *sout;
   objectptr cthis;
...
bool HierNameToObject(objinstptr cinst, char *hiername, pushlistptr *stack)
{
   CalllistPtr calls;
   ...
   while (hptr != NULL) {
	 ...
	 return false; 
      ...
      push_stack(stack, calls->callinst);
...
static void topflat(objectptr cschem, objinstptr thisinst, CalllistPtr cfrom,
        const char *prefix, FILE *fp, const char *mode)
{
...
void resolve_devindex(objectptr cschem, bool autonumber)
{
   CalllistPtr calls;
   char *stmp, *endptr;
   const char *idxtype[] = {"index", "idx", NULL};
   oparamptr ops, ips;
   objinstptr cinst;
   stringpart *optr;
   int newindex, j;
...
void resolve_indices(objectptr cschem, bool autonumber) {
   CalllistPtr calls;
        ...
        FILE *fp, const char *mode)
{
   CalllistPtr calls = cschem->calls;
   PortlistPtr ports, plist;
   int pnet, portid, length, plen, subnet, modlen;
   char *locmode = NULL, *stsave = NULL, *pstring;
   stringpart *ppin;
   Calllist loccalls;

   if (cschem->traversed) return;
...
int updatenets(objinstptr uinst, bool quiet) {
   ...
   if (checkvalid(thisobject) == -1) {
      ...
      if (areawin->selects > 0)
         ssave = new uselection(areawin->topinstance, areawin->selectlist,
		areawin->selects);
      destroynets(thisobject);
      createnets(thisinst, quiet);
      if (areawin->selects > 0) {
         areawin->selectlist = ssave->regen(areawin->topinstance);
         delete ssave;
      }
   }
...
void writenet(objectptr thisobject, const char *mode, const char *suffix)
{
   objectptr cschem;
   objinstptr thisinst;
   char filename[100];
   char *prefix, *cpos, *stsave = NULL;
   const char *locmode = mode;
   FILE *fp;
   bool is_spice = false, sp_end_save = spice_end;
   ...
   *prefix = '\0';
   ...
   if (is_spice && spice_end) fprintf(fp, ".end\n");
   ...
   for (calls = cschem->calls; calls != NULL; calls = calls->next) {
	 ...
	 while ((rhs = strchr(sptr, '=')) != NULL) {
	    ...
	    *rend = '\0';
	    ...
	    if (implicit == NULL) {
	       stringpart *strptr;
               label templabel(GLOBAL, XPoint(0,0));
...
bool RemoveFromNetlist(objectptr thisobject, genericptr thiselem)
{
...
int cleartraversed_level(objectptr cschem, int level)
{
   ...
   for (objinstiter cinst; pschem->values(cinst); ) {
...
void freetemplabels(objectptr cschem)
{
   ...
   for (cgen = cschem->begin(); cgen != cschem->end(); cgen++) {
      if (IS_OBJINST(*cgen)) {
	 ...
	 if (clab->string->type != FONT_NAME) {
	    ...
	    clab = TOLABEL(cgen);
            delete clab;
            for (tgen = cgen + 1; tgen < cschem->end(); tgen++)
	       *(tgen - 1) = *tgen;
	    cschem->parts--;
	    cgen--;
	 }
...
bool check_included(char *filename)
{
