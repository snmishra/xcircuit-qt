...
static PyObject *xc_new(PyObject *self, PyObject *args)
{
   ...
   switch(type = string_to_type(etype)) {
      case ARC:
         newgen = topobject->append(new arc(0, 0));
	 break;
      case SPLINE:
         newgen = topobject->append(new spline(0,0));
	 break;
      case POLYGON:
         newgen = topobject->append(new poly(4, 0, 0));
	 break;
      case LABEL:
         newgen = topobject->append(new label(0, XPoint(0,0)));
	 break;
      case PATH:
         newgen = topobject->append(new path(0, 0));
	 break;
      case OBJINST:
         newgen = topobject->append(new objinst(NULL, 0, 0));
	 break;
      default:
...
int PyRGBToIndex(PyObject *cobj)
{
   ...
   if (PyTuple_Check(cobj) && PyTuple_Size(cobj) == 3) {  /* RGB 3-tuple */
      ...
      if (PyFloat_Check(tobj)) {
         r = (int)(255 * (float)PyFloat_AsDouble(tobj));
         g = (int)(255 * (float)PyFloat_AsDouble(PyTuple_GetItem(cobj, 1)));
         b = (int)(255 * (float)PyFloat_AsDouble(PyTuple_GetItem(cobj, 2)));
      }
      else if (PyInt_Check(tobj)) {
         r = (int)(PyInt_AsLong(tobj));
         g = (int)(PyInt_AsLong(PyTuple_GetItem(cobj, 1)));
         b = (int)(PyInt_AsLong(PyTuple_GetItem(cobj, 2)));
      }
      else {
      ...
      ccolor = getnamedcolor(PyString_AsString(cobj));
   }
   ...
   switch(ELEMENTTYPE(*gelem)) {
	 ...
	 if ((dval = PyDict_GetItemString(attrdict, "points")) != NULL) {
	       ...
	       int number = PyList_Size(dval);
               if (TOPOLY(gelem)->points.count() != number) {
                  TOPOLY(gelem)->points.resize(number);
	       }
	       for (i = 0; i < number; i++) {
		  pval = PyList_GetItem(dval, i);
		  if (PyTuple_Check(pval) && PyTuple_Size(pval) == 2) {
...
static PyObject *xc_refresh(PyObject *self, PyObject *args)
{
   ...
   while (XCheckWindowEvent areawin->viewport, ~NoEventMask, &event))
...
static PyObject *xc_newtool(PyObject *self, PyObject *args)
{
   ...
   if (XpmReadFileToImage (char *)pixfile, &iret, NULL, &attr) < 0) {
      Wprintf("Cannot find or open pixmap file \'%s\'", pixfile);
      XpmCreateImageFromData q_xpm, &iret, NULL, &attr);
   }
